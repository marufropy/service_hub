<?php 
	...
	if ( null==$Service_ID || null==$Service_Type ) {
		...
		$sql = "SELECT service_category.Service_ID,provider.Provider_ID,provider.Name,provider.Working_Since
				FROM provider INNER JOIN service_category ON provider.Service_CategoryService_ID=service_category.Service_ID
				WHERE Service_ID = '$Service_ID'
				ORDER BY provider.Working_Since ASC";
		$q = $pdo->prepare($sql);
		$q->execute();
